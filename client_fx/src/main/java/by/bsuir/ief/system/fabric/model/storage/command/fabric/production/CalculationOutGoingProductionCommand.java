package by.bsuir.ief.system.fabric.model.storage.command.fabric.production;

import by.bsuir.ief.system.fabric.model.entity.fabric.CalculationOutGoingProductionEntity;
import by.bsuir.ief.system.fabric.model.storage.ConnectToServer;
import by.bsuir.ief.system.fabric.model.storage.command.AbstractCommand;
import com.google.gson.Gson;
import connection.client_support.Request;
import connection.client_support.requestbody.NoContentRequestBody;
import connection.client_support.requestbody.StringRequestBody;
import io.reactivex.Observable;
import io.reactivex.rxjavafx.schedulers.JavaFxScheduler;

public class CalculationOutGoingProductionCommand extends AbstractCommand<CalculationOutGoingProductionEntity> {
    private final int id;

    public CalculationOutGoingProductionCommand(int id) {
        this.id = id;
    }

    @Override
    protected Request getRequest() throws Exception {
        Request.Builder builder = new Request.Builder()
                .body(new StringRequestBody(String.valueOf(id)))
                .addMethod("action.cost.outgoing.production");

        builder = ConnectToServer.setConnectData(builder);

        return builder.build();
    }

    @Override
    public Observable<CalculationOutGoingProductionEntity> execute() {
        return super.getObservable()
                .map(response -> new Gson().fromJson(new String(response.getBody().bytes()), CalculationOutGoingProductionEntity.class))
                .observeOn(JavaFxScheduler.platform());
    }
}
